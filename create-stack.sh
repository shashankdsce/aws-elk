...
aws s3 cp ./master.yml "s3://$bucket_name/elk/" --region "$region"  --profile $AWS_DEFAULT_PROFILE

aws s3 cp --recursive cloudformation/ "s3://$bucket_name/elk/cfn/" --region "$region"  --profile $AWS_DEFAULT_PROFILE

aws s3 cp --recursive config/ "s3://$bucket_name/elk/config/" --region "$region" --profile $AWS_DEFAULT_PROFILE
...
aws cloudformation update-stack \
  ...
  --parameters \
    ...
    ParameterKey=AllowedSshCidr,ParameterValue="172.16.141.0/25" \
    ...
    ParameterKey=ElkCapacity,ParameterValue=2 \
    ...
    ParameterKey=PublicVpcSubnetA,ParameterValue="subnet-02cc4967" \
    ParameterKey=PublicVpcSubnetB,ParameterValue="subnet-8471d2f3" \
    ParameterKey=PrivateVpcSubnetA,ParameterValue="subnet-0dcc4968" \
    ParameterKey=PrivateVpcSubnetB,ParameterValue="subnet-8571d2f2" \
    ParameterKey=SnapshotRepository,ParameterValue="devisland-elk-backup" \
    ParameterKey=Stack,ParameterValue="elasticsearch" \
    ParameterKey=NodeMaster,ParameterValue="true" \
    ParameterKey=NodeData,ParameterValue="true" \
    ParameterKey=KibanaStack,ParameterValue="kibana" \
    ParameterKey=LogstashStack,ParameterValue="logstash" \
